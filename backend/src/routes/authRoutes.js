...
router.post('/record-vote', protect, async (req, res) => {
    try {
        ...
        // SECURE VOTE ANCHORING: Atomic update to prevent race conditions or duplicate tallies
        const voteKey = `${electionId}-${role}`;

        // Check if user has already voted for THIS specific role in THIS election
        if (req.user.votedElections.includes(voteKey) && req.user.role !== 'Admin') {
            return res.status(403).json({ message: `Access Denied: Vote already anchored for role ${role}.` });
        }

        // We only set the global isVoted to true if the user has completed ALL active elections
        // For BMS College Elections, there are 9 clubs, each with 3 roles (Total 27 votes)
        const updatedVotedElections = [...req.user.votedElections, voteKey];
        
        // Count how many unique elections have at least 3 roles voted
        const completedElections = new Set();
        const electionVoteCounts = {};

        updatedVotedElections.forEach(vKey => {
        ...
        // The system is considered "fully voted" only when all 9 clubs are completed
        const shouldMarkAsVoted = completedElections.size >= 9;

        const updatedUser = await User.findOneAndUpdate(
            { _id: req.user._id },
            {
                $set: { isVoted: shouldMarkAsVoted },
        ...
        if (!updatedUser) {
            return res.status(404).json({ message: 'Update failed: User record corrupted or session expired.' });
        }

        console.log(`[VOTEX SERVER] Vote recorded for User: ${updatedUser.fullName} in Role: ${role}`);
